...
@php
if ($type == 'assessment'){
       $file_name = 'imageQuestion';
       $title = t('Lesson Assessment Preview');

}elseif($type == 'training'){
       $file_name = 'imageQuestion';
       $title = t('Lesson Training Preview');
                        ...
                        <form  id="term_form" method="post">
                            ...
                            <div class="question-list">
                                ...
                                @foreach($questions as $question)
                                    @if($question->type == 1)
                                        @include('general.user.lesson.parts.true_false')
                                    @elseif($question->type == 2)
                                        @include('general.user.lesson.parts.options')
                                    @elseif($question->type == 3)
                                        @include('general.user.lesson.parts.match')
                                    @elseif($question->type == 4)
                                        @include('general.user.lesson.parts.sort')
                                    @endif
                                    @php
                                        $counter ++;
                                    @endphp
