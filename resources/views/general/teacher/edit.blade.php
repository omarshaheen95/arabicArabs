...
@section('content')
    <form
        action="{{ isset($teacher) ? route(getGuard().'.teacher.update', $teacher->id): route(getGuard().'.teacher.store') }}"
        ...
        <div class="row">
            ...
            <div class="row">
                <div class="col-6 mb-2">
                    <div class="form-group">
                        ...
                        <input type="url" id="School_Name" name="name" class="form-control" placeholder="{{t('Name')}}"
                               value="{{ isset($teacher) ? $teacher->name : old("name") }}" required>
                    </div>
                </div>
                ...
                <div class="col-6 mb-2">
                    ...
                    @include('components.force-password-change', ['row' => $teacher ?? null])
                </div>
                ...
                <div class="col-6 mb-2">
                    <div class="form-group">
                        ...
                        <input type="tel" id="mobile" name="mobile" class="form-control" placeholder="{{t('Mobile')}}"
                               value="{{ isset($teacher) ? $teacher->mobile : old("mobile") }}" required>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="school_id" class="form-select" data-control="select2"
                                ...
                                @foreach($schools as $school)
                                    <option
                                        value="{{$school->id}}" {{isset($teacher) && $teacher->school_id==$school->id?'selected':''}}>{{$school->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="col-6 mb-2">
                    <div class="form-group">
                        ...
                        <input class="form-control datePiker" name="active_to" type="text"
                               value="{{ isset($teacher) ? $teacher->active_to : old("active_to") }}" id="active_to"
                ...
                <div class="col-6 mb-2 d-flex gap-2 mt-4">
                    <div class="form-check form-check-custom form-check-solid">
                        <input class="form-check-input" type="checkbox" value="1" name="active"
                               id="flexCheckDefault" {{isset($teacher) && $teacher->active ? 'checked':''}}/>
                    ...
                    @if(guardIs('manager'))
                        <div class="form-check form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" value="1" name="approved"
                                   id="flexCheckDefault" {{isset($teacher) && $teacher->approved ? 'checked':''}}/>
                            <label class="form-check-label text-dark" for="flexCheckDefault">
                                {{t('Approved')}}
                            </label>
                        </div>
            ...
            <div class="row my-5">
                ...
                <div class="col-12 d-flex justify-content-end">
                    <button type="submit"
                            class="btn btn-primary mr-2">{{isset($teacher)?t('Update'):t('Submit')}}</button>
                </div>
            </div>
        </div>
...
@section('script')
    ...
    {!! JsValidator::formRequest(\App\Http\Requests\General\TeacherRequest::class, '#form-data'); !!}
    <script>
