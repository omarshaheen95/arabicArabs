...
class LessonAssignmentController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $rows = UserAssignment::query()->has('user')->with(['user.school','user.grade', 'user.teacher', 'lesson.grade'])->filter($request)->latest();
        ...
        return view('manager.lessons_assignments.index', compact('title','grades', 'schools'));
    }
    ...
    public function store(LessonAssignmentRequest $request)
    {
        ...
        return redirect()->route('manager.lesson_assignment.index')->with('message',t('Successfully Added'));
