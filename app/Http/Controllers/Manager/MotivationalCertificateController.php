...
use App\Http\Requests\Manager\MotivationalCertificateRequest;
...
class MotivationalCertificateController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (request()->ajax())
        {
            $rows = MotivationalCertificate::query()
                ->with(['model', 'user.school','user.grade', 'teacher'])
            ...
            return DataTables::make($rows)
                ...
                ->addColumn('student', function ($row){
                    $student =  '<div class="d-flex flex-column">'.
                        ...
                        '<div class="d-flex"><span class="fw-bold text-primary pe-1">'.t('School').':</span>'.optional($row->user)->school->name.'</div>'.
                ...
                ->addColumn('dates', function ($row){
                    $student =  '<div class="d-flex flex-column">'.
                        '<div class="d-flex"><span class="fw-bold text-primary pe-1">'.t('Teacher').':</span>'.$row->teacher->name.'</div>'.
        ...
        $schools = School::query()->filter(request())->get();
        $grades = Grade::query()->get();
        return view('manager.motivational_certificates.index', compact('title', 'schools','grades'));
    }
    ...
    public function create()
    {
        ...
        $schools = School::query()->filter(request())->get();
        $grades = Grade::query()->get();
        return view('manager.motivational_certificates.edit', compact('title', 'grades','cer_type', 'schools'));
    }
    ...
    public function store(MotivationalCertificateRequest $request)
    {
        ...
        return redirect()->route('manager.motivational_certificate.index')->with('message', t('Successfully Added'));
    }
