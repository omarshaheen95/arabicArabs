...
$(document).ready(function () {
    ...
    $('.question-reader').on('click',function (e) {
        e.preventDefault();
        initAudio($(this))
    })
});
...
//Question Reader Audio Player -----------------------------------------------------------------------

let audio = null;
let lastQuestionReader = null;
let activeIconUrl = null;
let unActiveIconUrl = null;
function initAudio(currentElement){
    if (audio) {
        pauseAudioOrPlay(currentElement)
    }else {
        playAudio(currentElement)
    }
}
function playAudio(currentElement) {
    let url = currentElement.data('src');
    if (!activeIconUrl && !unActiveIconUrl){
        activeIconUrl = currentElement.data('active-icon')
        unActiveIconUrl = currentElement.attr('src')
    }
    lastQuestionReader = currentElement;
    currentElement.attr('src',activeIconUrl)
    audio = new Audio(url);
    audio.play()

    audio.addEventListener('ended', () => {
        //on audio end
        lastQuestionReader.attr('src',unActiveIconUrl)
    });

}
function pauseAudioOrPlay(currentElement){
    if (currentElement.attr('id') === lastQuestionReader.attr('id')){
        lastQuestionReader.attr('src',unActiveIconUrl)
        audio.pause();
        audio.currentTime = 0;
        audio = null;
